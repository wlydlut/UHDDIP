...
def paired_random_crop_FP(img_lqLs, img_lqRs, img_gts, img_masks, gt_patch_size, scale, gt_path):
    if not isinstance(img_gts, list):
        img_gts = [img_gts]
    ...
    top_mask, left_mask = int(top * scale), int(left * scale)
    img_masks = [
        v[top_mask:top_mask + gt_patch_size, left_mask:left_mask + gt_patch_size, ...]
        for v in img_masks
    ]
    if len(img_gts) == 1:
        img_gts = img_gts[0]
    ...
    if len(img_lqRs) == 1:
        img_lqRs = img_lqRs[0]
    return img_lqLs, img_lqRs, img_gts, img_masks
...
def random_augmentation(*args):
    out = []
    flag_aug = random.randint(0,7)  #(0,1)
    for data in args:
