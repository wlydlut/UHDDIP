...
def paired_FP_paths_from_folder(folders, keys, filename_tmpl):
    """Generate paired paths from folders.
    ...
    assert len(keys) == 4, (
        'The len of keys should be 2 with [inputL_key, inputR_key, gt_key]. '
        f'But got {len(keys)}')
    inputL_folder, inputR_folder, mask_folder, gt_folder, = folders
    inputL_key, inputR_key, mask_key, gt_key= keys

    inputL_paths = list(scandir(inputL_folder))#input
    inputR_paths = list(scandir(inputR_folder))#normal
    mask_paths = list(scandir(mask_folder))#mask
    gt_paths = list(scandir(gt_folder))#gt

    assert len(inputL_paths) == len(inputR_paths) == len(gt_paths) == len(mask_paths), (
        f'{inputL_key} and {inputR_key} and {gt_key} and {mask_key}datasets have different number of images: '
        f'{len(inputL_paths)}, {len(inputR_paths)}, {len(gt_paths)},{len(mask_paths)}.')
    paths = []
    for idx in range(len(gt_paths)):
        gt_path = gt_paths[idx]
        basename, ext = osp.splitext(osp.basename(gt_path))
        ...
        mask_name = f'{filename_tmpl.format(basename)}{ext_mask}'
        mask_path = osp.join(mask_folder, mask_name)
        assert mask_name in mask_paths, (f'{mask_name} is not in '
                                         f'{mask_key}_paths.')
